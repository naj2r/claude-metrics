...
## 2026-05-11 — MCP-Stata wedge from Windows multiprocessing orphans — `python -m mcp_stata` embeds Stata via pystata + `multiprocessing.spawn`; when the parent dies on Windows, the multiprocessing children survive (no `PR_SET_PDEATHSIG`) and each holds ~140 MB of in-process Stata plus a claim on the single-user license. After a week of interrupted Claude Code sessions, 7 orphans had accumulated (3 with dead parents, 2 active mcp_stata duplicates + 2 workers). Every new MCP `run_command` wedged indefinitely because pystata was blocked on a license slot. — Wrote safe-launch wrapper at `$DROPBOX/Scripts/start_mcp_stata.ps1` that sweeps stale processes (cmdline match on `*mcp_stata*` OR `*multiprocessing-fork*` with RSS >100MB) before launching `python -m mcp_stata`. Added "MCP-Stata process leak (Windows-specific)" section to `.claude/rules/stata-gotchas.md` covering symptoms, detection PowerShell, and wire-up instructions. User must re-register the `stata` MCP server to point at the wrapper and restart Claude Code for the safeguard to take effect.
